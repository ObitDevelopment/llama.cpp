...
// Obit fork: stage-aware ABI lives in its own header so upstream merges
// against this file essentially never conflict. Pulled in here for
// back-compat with consumers that only `#include "llama.h"`. Consumers
// may also `#include "llama-obit-stage.h"` directly.
#include "llama-obit-stage.h"
